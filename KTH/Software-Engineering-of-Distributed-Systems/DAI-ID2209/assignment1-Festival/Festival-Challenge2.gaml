...
global {
	...
	init {
		...
		create Guard number: numOfGuards;
	}
}
...
species Guard skills: [moving]{
	rgb guardColor <- #cyan;	
	GuestWithBrain targetGuest <- nil;
	
	aspect base {
		draw cube(2) color: guardColor;
	}
	
	reflex findGuest when: targetGuest != nil {
		do goto target:targetGuest;
	}
	
	reflex removeGuest when: targetGuest != nil and location distance_to(targetGuest) = 0 {
		ask GuestWithBrain at_distance(0){
			do die;
...
species InformationCenter {
	...
	GuestWithBrain bad_apple <- nil;
	
	aspect base {
		draw cube(5) color: #red ;
	}
	
	reflex checkForBadApple when: bad_apple = nil{
		ask agents of_species GuestWithBrain at_distance(1){
			if(self.bad_apple = true){
				myself.bad_apple <- self;
			}
		} 
	}
	reflex callGuard when: bad_apple != nil{
		write "GUARD!!!";
		ask Guard {
			self.targetGuest <- myself.bad_apple;
