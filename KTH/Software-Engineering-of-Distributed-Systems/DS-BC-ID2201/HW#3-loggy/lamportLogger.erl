...
init(Nodes) ->
  ...
  Clock = lamportTime:clock(Nodes),
  % defining initial hold back queue to be empty list
  HoldBackQueue = lamportTime:newQueue(),
  % loop method
...
loop(Clock,HoldBackQueue) ->
  receive
    {log, From, Time, Msg} ->
      ...
      UpdatedClock = lamportTime:update(From,Time,Clock),
      % add this new message to hold-back queue.
      UpdatedHoldBackQueue = lamportTime:updateHoldBackQueue(From,Time,Msg,HoldBackQueue),
      % now we log the messages that are safe to log from the queue and return unsafe queue so that we can pass it back
      UnSafeHoldBackQueue = lamportTime:logSafeHoldBackQueue(UpdatedClock,UpdatedHoldBackQueue),
